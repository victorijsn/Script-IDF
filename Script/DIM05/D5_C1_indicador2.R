...
# Indicador: 5.1.2. Ausência de criancas menores de 14 anos trabalhando

D5_C1_I2 <- function(base){
  ...
  dado[aux_idade<14L, marca_14anos_trab:=fifelse(aux_pessoa_ocupada==1L |
                                                   p.ind_trabalho_infantil_pessoa==1L,
                                                 1L, 0L)]
  
  # calculando o indicador --------------------------------------------------
  
  dado <- dado[, .(total_14anos_trab = 
                     sum(marca_14anos_trab, na.rm = TRUE)),
               by = c("d.cod_familiar_fam")]
  
  dado[, d5_c1_i2 := fifelse(total_14anos_trab > 0, 0, 1)]
