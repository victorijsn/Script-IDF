...
#'
#' @examples
IDF_clean2 <- function(ExtPobreza = 249.5, SalarioMin = 998, tudo, Ano_inicial= 2016, Ano_final = 2019, Escolhe_base = "2019-03", LimSupPobreza = 499){
...
b <- as.data.table(b)
collapse1<- b[ , by = c("cod_familiar_fam","cd_ibge"), lapply(.SD, mean, na.rm=TRUE),
                                          .SDcols = c(
"idade",                           "pes_cad",                          "index",
...
"renda",                           "renda_ntrans",                     "mes3",
"ano3",                            "inpc",                             "INDICE",
"INDICE_BASE_DIV",                 "INDICE_BASE_MULT",                 "deflatores",
"renda_defla",                     "aux_t5",                           "aux_t6",
"defas",                           "aux_d1",                           "aux_d2",
"aux_d3",                          "aux_d4",                           "aux_d5",
"aux_d6",                          "aux_d7",                           "aux_d8")]

collapse1a <- b[,by = c("cod_familiar_fam","cd_ibge"), lapply(.SD, sum, na.rm = TRUE),
       .SDcols = c("aux_v8a_", "aux_v8b_", "aux_t1_", "aux_t2_", "renda_transf_")]
#B <- cbind(collapse1,collapse1a) #nova_base_pessoas
B <- left_join(collapse1, collapse1a, by=c("cd_ibge"="cd_ibge", "cod_familiar_fam"="cod_familiar_fam"))
...
#Preparando Saídas
out <- select(J,cod_familiar_fam,idf)
